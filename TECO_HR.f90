...
!   Some initial settings
    integer,parameter :: simu_n= 100000  ! number of simulation for DA
...
!   Time series
    ...
    integer, parameter :: day_length = 1461!1826  !5114 ! 1096 ! determine size of blank array !Aneesh: modified it for 2017 to 2020
...
!   Switches *************************************************************************************
    ...
    logical, parameter :: do_snow   = .false.!.true.    ! if true, model simulate snow; else read snow from observation files

!   Switches for hourly write out
    logical, parameter :: wrt_cflux_d=.true.
    logical, parameter :: wrt_swater_d=.true.
    logical, parameter :: wrt_tsoil= .true.
    logical, parameter :: wrt_energy=.true.  !.false.
    logical, parameter :: wrt_nflux=.false.
    logical, parameter :: wrt_swaterflux= .true.      !.false.
    logical, parameter :: wrt_pheno=.false.
    logical, parameter :: wrt_cdist=.true. !.false.
...
!    namelist
    ...
    namelist /control_pm/climatefile,forcingdir,parafile,parafile_siteconditions,parafile_sconditions, &
            ...
            outdir,outdir_simu,outdir_da,outdir_forecast, outdir_da_cflux, outdir_da_swater, &
...
!    nml_file = 'namelist_Ses2.nml'
    ...
    write(*,*)'outdir_da_cflux=',outdir_da_cflux
    write(*,*)'outdir_da_swater=',outdir_da_swater
...
! Aneesh: it creates only header of the file
        ...
        if(wrt_swater_d) then
            ...
            &duplicate_swc(1),duplicate_swc(2),duplicate_swc(3),duplicate_swc(4)'
...
!                    &   cm3/cm3,cm3/cm3,cm3/cm3,cm3/cm3'
        ...
        if (wrt_swaterflux) then
            ...
            write(3007,*) 'm,water_free,runoff,water_leach,evap,transp,plantup(1),plantup(2),plantup(3),plantup(4),&
                             &  plantup(5),plantup(6),plantup(7),plantup(8),plantup(9),plantup(10),&
                             &  waterF(1),waterF(2),waterF(3),waterF(4),&
...
!           Get observation data
            if(obs_nee_file.ne.'')then
                ...
                allocate(obs_nee_d_date(0))
                allocate(obs_nee_d(0))
            endif
            ...
            if(obs_abc_file.ne.'')then
                ...
                allocate(obs_abc_d_date(0))
                allocate(obs_abc_d(0))
            endif
...
!   Switches
    ...
    integer,parameter:: vegetype=2          !Aneesh: Changed to 2 from 1 for woody species
    integer rcp
...
!   Soil_water subroutine --------------------------------------
    ...
    real wsmin_fr,swc_fr,waterF(10),wpotent(10),duplicate_swc(10)
    real runoff,water_free,water_leach,plantup(10)
    real Rsoil
...
!                print*,"Vcmax_n:",Vcmax_n
                ...
                &   waterF,wpotent,water_leach,plantup,duplicate_swc)   ! outputs
...
!               Write out soil water fluxes ---------
                    if (wrt_swaterflux.and.MCMC.eq.0) then
                        write (3007,30071) m,water_free,runoff,water_leach,evap,transp,plantup(1),plantup(2),plantup(3),plantup(4),&
                             &  plantup(5),plantup(6),plantup(7),plantup(8),plantup(9),plantup(10),&
                             &  waterF(1),waterF(2),waterF(3),waterF(4),&
                             &  waterF(5),waterF(6),waterF(7),waterF(8),waterF(9),wpotent(1),wpotent(2),wpotent(3),&
                             &  wpotent(4),wpotent(5),wpotent(6),wpotent(7),wpotent(8),wpotent(9),wpotent(10)
                    endif
30071               format(1(I8,","),37(f40.20,","),(f40.20))
...
!                     if(wrt_swater_d) then
!                         write(3002,30021) swc(1),swc(2),swc(3),swc(4),swc(5),swc(6),swc(7),swc(8),swc(9),swc(10),&
!                             & duplicate_swc(1),duplicate_swc(2),duplicate_swc(3),duplicate_swc(4) !! YZhou: addedd another five layers from 6 to 10
!                     endif
! 30021               format (13(f15.10,","),(f15.10))
...
!   Outputs
    ...
    real waterF(10),WPotent(10),water_leach, duplicate_swc(10)
...
!   Melting ---------------------------------------------------------------------
    ...
    !    !water_free = 0.0
    !endif
    water_free=water_free+rain



!   Rainfall ------------
!! YZhou: infiltration could/should be calculated with potential head, but introducing it caused the infiltration rate much too high... One reason could be unit. Need to check the unit of rainfall (mm/??).
	!if (water_free .gt. 0) then
		!n_pot(1) = 2.06
		!mpara(1)=1-1/n_pot(1)

		!relsat(1)=Amax1(0.0001,Amin1(0.9999,(swc(1)-waterres(1))/(watersat(1)-waterres(1))))
		!Wpotent(1)=Amin1(-relsat(1)**(1/(-wlama(1)))/potcof(1),0.0)*10200 ! Mpa to cm
    ...
    wsc_emp(1) = Amax1((wsmax(1)-swc(1))*sthick(1)*10.0,0.0)  ! water storage capacity

    if (water_free.le.wsc_emp(1)) then
        swc(1)= swc(1)+water_free/(sthick(1)*10.0) !mm3 mm-3
        water_free=0.0
    else
        water_free=water_free-wsc_emp(1)
        swc(1)=wsmax(1)
        !swc(2)=Amin1(swc(2)+water_free/(sthick(2)*10.0), wsmax(2))
        !water_free=0!water_free-swc(2)
    endif

!Aneesh: Uncommenting runoff code block to calculate runoff
!   Runoff --------------
...
!   Unit of transp: mm/h
    do i=1,layern
        relwater(i)=Amax1(0.00,(swc(i)-waterres(i))*sthick(i)*10.0) ! unit: mm          !relative water content of soil
    enddo
...
!   n_pot = 2.0 ! decrease n_pot there will be a smaller decrease in swc when swc is high
    ...
    do i=1,(layern-1)
        relsat(i)=Amax1(0.0001,Amin1(0.9999,(swc(i)-waterres(i))/(watersat(i)-waterres(i)))) ! relative soil water saturation, dimensionless
        ...
        !Aneesh: Brooks and corey Model 1964 Brooks, R. H. (1965). Hydraulic properties of porous media. Colorado State University.
        Wpotent(i)=Amin1(-potcof(i)*(relsat(i))**(-(wlama(i))),0.0) ! Mpa
        Wpotent(i+1)=Amin1(-potcof(i+1)*(relsat(i+1))**(-(wlama(i+1))),0.0)

        ! Wpotent(i) = -1/(10200*alpha_gen(i)) * & 
        ! ((((watersat(i) - waterres(i))/(swc(i) - waterres(i)))**(1/mpara(i)))-1)**(1/n_pot(i))             
		! Wpotent(i+1) = -1/(10200*alpha_gen(i+1)) * & 
        ...
        condval(i)=condsat(i)*relsat(i)**(3+2*wlama(i))   ! cm h-1
        condval(i+1)=condsat(i+1)*relsat(i+1)**(3+2*wlama(i+1))

        WpotentDT=(WPotent(i)-WPotent(i+1))/((sthick(i+1))) !cm cm-1               !! Aneesh: Unit of WpotentDT is cm
        ...
        if (WpotentDT.ge.0) then
            waterF(i)=condval(i)*(WpotentDT+1) ! unit: cm/h. Since WPotent is negative, waterF is negative.
        else
            waterF(i)=condval(i+1)*(WpotentDT+1) ! unit: cm/h. Since WPotent is negative, waterF is negative.
        endif

        !print*,i,WPotent(i),waterF(i), relsat(i),wlama(i),potcof(i)
        ! swc(i)=swc(i)-waterF(i)/(sthick(i)*10)              !Aneesh: Added Amax1 so that when swc is less than waterres, waterres value is stored
        ! swc(i+1)=swc(i+1)+waterF(i)/(sthick(i+1)*10)
        swc(i)=Amax1(swc(i)-waterF(i)/(sthick(i)), waterres(i))              !Aneesh: Added Amax1 so that when swc is less than waterres, waterres value is stored
        swc(i+1)=Amin1(swc(i+1)+waterF(i)/(sthick(i+1)), wsmax(i+1))
        !print*,i,Wpotent(i),Wpotent(i+1),WpotentDT,condval(i),condval(i+1),waterF(i),sthick(i)
        ! if(swc(i).lt.waterres(i)) then
        !     print*,i,swc(i),waterF(i),WPotent(i),WPotent(i+1),WPotentDT, waterres(i)
        !     print*,"Warning: Something is wrong with unsaturated soil water conductance!!!"
        !     stop
        ! endif
    enddo
    
    do i = 1, layern                            !!! storing soil water content before HR to estimate the HR
        duplicate_swc(i) = swc(i)
    end do
...
!   Hi: water move into soil layer i from all other layers
    ...
    if(do_hydr_redist) then
        ...
        do i=1,10
            do j=1,10
				...
				Fc(i) = 1/(1+(WPotent(i)/(-102))**3.22)              !Aneesh: replaced 3.22 by 1           
				Fc(j) = 1/(1+(WPotent(j)/(-102))**3.22)         !Aneesh:102 is the unit conversion factor from m to MPa
				! YZhou: end of adding
    ...
    !print*, radsol, duplicate_swc(1)-swc(1)

!   Leaching ----------------------------------------------------------------
    water_leach=Amax1((swc(layern)-wsmax(layern))*sthick(layern)*10*0.0005,0.0)  ! unit of mm/h   !Aneesh: remvoing 10*0.0005 by 10 only and changed back to wsmin from wsmax.
...
!   Water table --------------------------------------------------------------
    ...
    zmax = 3000   !maximum water table depth   mm        !Aneesh:Changed from 300 to 3000 mm
...
!   Update C/N ratio --------------------------------------
    do j=1,8            !Aneesh: changed to 8 from 10
        if(Npool(j).eq.0)then
            CN(i)=CNini(i)
        else
            CN(i-1)=Cpool(i-1)/Npool(i-1)       !Aneesh: changed to (i-1) from i
        endif
    enddo
...
!    print*,"delta:",sftmp,delta,resdh,drsdh,RsoilabT,Hsoil,Esoil,G
    ...
    do i=1,layern                      !Aneesh: changed to (layern-1) from layern because sthick should be 10 !Aneesh: removed (layern-1) as without it also now working      
...
!        print*,"tsol_diag 5_2:",i,stemp(i)
        ...
        if (isnan(stemp(i))) then
            print*,"soil temperature:",i,heat_excess,drsdh,thd_t,stemp(i), stemp(1), stemp(2), stemp(3)
...
subroutine one_layer(m,ng,yr,yr_spinup,phenoset,radabv,Qabs,Eabsorp_leaf_ng,grn,wind_l,Tair,TairK,Dair,      &    ! input
    ...
    implicit none
!    Switches
    logical,parameter:: wrt_Aleaf=.true.
...
!   Initilization
    ...
    if (do_swater_da) then
        ...
        if(obs_swc12p5_d_n.gt.0) then
            J_swc_d(2)=calJ(obs_swc12p5_d_n,simu_swc_d(2,obs_swc12p5_d_date),obs_swc12p5_d,obs_swc_d_var(2),fixedprop)*2
        endif
        if(obs_swc22p5_d_n.gt.0) then
            J_swc_d(3)=calJ(obs_swc22p5_d_n,simu_swc_d(3,obs_swc22p5_d_date),obs_swc22p5_d,obs_swc_d_var(3),fixedprop)*3
        endif
        if(obs_swc37p5_d_n.gt.0) then
            J_swc_d(4)=calJ(obs_swc37p5_d_n,simu_swc_d(4,obs_swc37p5_d_date),obs_swc37p5_d,obs_swc_d_var(4),fixedprop)*4
        endif
...
!    endif
    ...
    print*,isimu,upgraded,';delta_J:',int(delta_J*10)/10,";J new:",int(J_new*10)/10,";  acct rate(%):",int(accR*100), J_swc_d
