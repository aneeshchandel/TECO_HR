...
!   Some initial settings
    integer,parameter :: simu_n= 1000  ! number of simulation for DA
...
!   n_pot = 2.0 ! decrease n_pot there will be a smaller decrease in swc when swc is high
    ...
    do i=1,(layern-1)
        ...
        swc(i)=swc(i)-waterF(i)/(sthick(i)*10)
        swc(i+1)=swc(i+1)+waterF(i)/(sthick(i+1)*10)
        !print*,i,swc(i), swc(i+1), Wpotent(i)*10200,Wpotent(i+1)*10200,WpotentDT,condval(i),waterF(i),sthick(i)
