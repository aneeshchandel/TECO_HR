...
!   Write out simulated data  ---------------------------------
        ...
        if(wrt_swater_d) then
            ...
            write(3002,*)'swc(1),swc(2),swc(3),swc(4),swc(5)'
...
30011               format(3(f15.8,","),(f15.10))
                    ...
                    if(wrt_swater_d) then
                        write(3002,30021) swc(1),swc(2),swc(3),swc(4),swc(5)
                    endif
30021               format (4(f15.10,","),(f15.10))
...
!   Internal variables 
    ...
    real Crt,Crt_ad,c(10),condval(10),condb(10),Dtran
...
!   Rainfall ------------
    ...
    if (water_free.le.wsc_emp(1)) then
        swc(1)= swc(1)+water_free/(sthick(1)*10.0)
        water_free=0.0
    else
...
!   n_pot = 2.0 ! decrease n_pot there will be a smaller decrease in swc when swc is high
    ...
    do i=1,(layern-1)
        ...
        Wpotent(i)=Amin1(-relsat(i)**(1/(-wlama(i)))/potcof(i),0.0)
        Wpotent(i+1)=Amin1(-relsat(i+1)**(1/(-wlama(i+1)))/potcof(i+1),0.0)
        
        condval(i)=condsat(i)*relsat(i)**0.5 *(1-(1-relsat(i)**(1/mpara(i)))**mpara(i))**2        
        condval(i+1)=condsat(i+1)*relsat(i+1)**0.5 *(1-(1-relsat(i+1)**(1/mpara(i+1)))**mpara(i+1))**2    
        
        WpotentDT=(WPotent(i)-WPotent(i+1))*10200/((sthick(i)+sthick(i+1))/2)+1
...
!        print*,i,WPotent(i),relsat(i),wlama(i),potcof(i)
...
!   Hi: water move into soil layer i from all other layers 
    ...
    if(do_hydr_redist) then
        ...
        do i=1,10
            do j=1,10
                if(Frlen(i).gt.Frlen(j))then
                    Frlenx(i)=Frlen(i)
                else
                    Frlenx(i)=Frlen(j)
                endif
                HRP_tem = Crt_ad*(WPotent(j)-WPotent(i))*Amax1(c(i),c(j))*(Frlen(i)*Frlen(j))/(1-Frlenx(i))*Dtran
...
!   Leaching ----------------------------------------------------------------
    water_leach=Amax1((swc(layern)-wsmin(layern))*sthick(layern)*10*0.0005,0.0)  ! unit of mm/h
...
!    print*,"Esoil:",Esoil,slope*(RsoilabT-G),RsoilabT
